...
module.exports = {
  ...
  getWithInfo: async (id) => {
    try {
      const sql =`SELECT JA.dateofcreation, JA.id, JD.title, JO.deadline, O.name, JD.description, JD.workload, JD.salary, JD.place, JD.supervisor, JO.numberofattachments
      FROM JobApplication as JA
      JOIN JobOffer as JO
      ON JA.idoffer = JO.id
