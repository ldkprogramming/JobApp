...
router.get('/:idApplicant', asyncHandler(async (req, res, next) => {
    res.render("applicant/home", {idApplicant: req.session.rolesIdMap.applicantId, idRecruiter: req.session.rolesIdMap.recruiterId});
}));
...
router.get('/:idApplicant/job-offers/published', asyncHandler(async (req, res, next) => {
    ...
    res.render('applicant/offers', {jobOffers: jobOffers, idApplicant: req.session.rolesIdMap.applicantId, idRecruiter: req.session.rolesIdMap.recruiterId});
}));
...
router.get('/:idApplicant/job-applications', asyncHandler(async (req, res, next) => {
    ...
    res.render("applicant/applications", { jobApplications: jobApplications, idApplicant: req.session.rolesIdMap.applicantId, idRecruiter: req.session.rolesIdMap.recruiterId });
}));

router.get('/:idApplicant/become-recruiter', asyncHandler(async (req, res, next) => {
    // a faire
    res.render("applicant/recruiter_registration_request", {idApplicant: req.session.rolesIdMap.applicantId, idRecruiter: req.session.rolesIdMap.recruiterId})
}));
